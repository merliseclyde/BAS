...
The two main function in `BAS` are `bas.lm` and `bas.glm` for implementing Bayesian Model Averaging and Variable Selection and have a syntax similar to the `lm` and `glm` functions respectively.  To illustrate using `BAS` on a simple example using the Zellner-Siow Cauchy prior enter
...
Run `demo("BAS.hald")` or `demo("BAS.USCrime")` or see the package vignette for more examples and description of the functions.

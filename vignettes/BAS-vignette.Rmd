...
`BAS` can perform weighted regression by supplying an optional weight vector that is of the same length as the response where the assumption is that the variance of the response is proportional to 1/weights.  The g-prior incporporates the 
weights in the prior covariance, 
$$
 \sigma^2 g (X_\gamma^T W X_\gamma)
$$
...
```{r read-climate}
climate = dplyr::filter(climate, proxy != 6) %>%
          dplyr::mutate(proxy=factor(proxy))
```
...
climate.bas = bas.lm(deltaT ~ proxy*poly(latitude, 2), data=climate,
                     ...
                     modelprior=uniform())
```
Examining the image of the top  $20$ models,
```{r climate-image}
image(climate.bas, rotate=F)
```

we see that any level of a factor may enter or drop from the model independently. If we want to force that all levels of a factor or interaction of a factor enter or leave simultaneously, then we may use the function `force.heredity.bas` to
post-process the output to include or exclude terms.

```{r force-herid}
climate.herid.bas = force.heredity.bas(climate.bas)
image(climate.herid.bas, rotate=F)
```
We now see that there are now 5 models, with the model with just `proxy` having the highest posterior model probability.

A future version of `BAS` will implement this through the model prior which should allow for more efficient sampling and reduced memory requirements when dealing with factors that have many levels.
